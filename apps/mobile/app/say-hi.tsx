...
export default function SayHiScreen() {
  ...
  useEffect(() => {
    ...
    if (typeof params.biometricVerificationId === 'string' && params.biometricVerificationId) {
      ...
      setInfoMessage(typeof params.biometricMessage === 'string' ? params.biometricMessage : t('arrival.faceVerified'));
      setError(null);
    } else if (params.biometricEnrollmentStatus === 'ENROLLED') {
      setFaceState('idle');
      setBiometricVerificationId(null);
      setInfoMessage(typeof params.biometricMessage === 'string' ? params.biometricMessage : t('arrival.faceEnrollComplete'));
      void refresh();
    }
  ...
  async function runLocationCheck() {
    ...
    try {
      ...
      setInfoMessage(t('arrival.locationCaptured', { accuracy: nextLocation.accuracyMeters }));
    } catch (nextError) {
      setLocationState('idle');

      if (isPreciseLocationError(nextError)) {
        if (nextError.code === 'LOCATION_PERMISSION_REQUIRED') {
          setError(t('arrival.locationPermissionRequired'));
          return;
        }

        if (nextError.code === 'PRECISE_LOCATION_REQUIRED') {
          setError(t('arrival.locationPreciseRequired'));
          return;
        }

        if (nextError.code === 'LOCATION_ACCURACY_TOO_LOW') {
          setError(t('arrival.locationAccuracyTooLow'));
          return;
        }
      }

      setError(nextError instanceof Error ? nextError.message : t('arrival.locationCaptureFailed'));
    }
  }
  ...
  async function runFaceCheck() {
    ...
    if (biometricPolicy.enrollmentStatus !== 'ENROLLED') {
      setInfoMessage(t('arrival.faceEnrollmentRequired'));
      router.push({
        pathname: '/biometric',
        params: {
          mode: 'enroll',
  ...
  function stepBadge(state: VerificationState) {
    if (state === 'checked') return t('arrival.checked');
  ...
  async function handleContinue() {
    ...
    try {
      await submitAttendanceAction('check-in', {
        ...capturedLocation,
        biometricVerificationId,
        notes: 'Mobile attendance check-in',
      });
      router.replace('/today' as never);
    } catch (nextError) {
      ...
      <View className="flex-1">
        {!started ? (
          <View className="flex-1 justify-between">
            <View className="overflow-hidden rounded-[40px] bg-[#f4efe7] px-6 pb-8 pt-8">
              <View className="absolute -left-14 -top-12 h-44 w-44 rounded-full bg-[#ffd35f]" />
              <View className="absolute left-24 top-16 h-48 w-48 rounded-full bg-[#ff5c8d]/70" />
              <View className="absolute -right-10 bottom-0 h-64 w-64 rounded-full bg-[#8b7bff]/65" />
              ...
              <Badge label={t('arrival.shiftBadge')} className="mb-6 bg-white/65 text-foreground" variant="muted" />

              <Text className="text-[40px] font-extrabold leading-[44px] text-white">{t('arrival.welcome')}</Text>
              <Text className="mt-4 max-w-[250px] text-[16px] leading-6 text-white/90">{t('arrival.subtitle')}</Text>

              <View className="mt-10 items-center">
                <View className="h-36 w-36 items-center justify-center rounded-full bg-white/70 shadow-float">
                  <Text className="text-[72px]">👋</Text>
                </View>
              </View>

              <View className="mt-10 rounded-[28px] bg-white/22 p-5">
                <Text className="text-[14px] font-bold uppercase tracking-[2px] text-white/80">{t('today.shiftTiming')}</Text>
                <Text className="mt-2 text-[28px] font-extrabold text-white">{shiftTime}</Text>
                <Text className="mt-2 text-[15px] text-white/80">{status?.location.name ?? 'Beauty Life HQ'}</Text>
              </View>
            </View>

            <View className="gap-3 pb-2 pt-6">
              <Text className="text-center text-[15px] leading-6 text-muted">{t('arrival.sayHiHint')}</Text>
              <Button fullWidth className="min-h-16 rounded-[30px]" label={t('arrival.sayHi')} onPress={() => setStarted(true)} size="lg" />
            </View>
          </View>
        ) : (
          <View className="flex-1 gap-5">
            <View className="gap-3">
              <Badge label={t('arrival.shiftBadge')} variant="brand" />
              <Text className="text-[32px] font-extrabold text-foreground">{t('arrival.verificationTitle')}</Text>
              <Text className="text-[16px] leading-6 text-muted">{t('arrival.verificationSubtitle')}</Text>
            </View>

            <Card className="gap-4 rounded-[32px] bg-white/72">
              <View className="flex-row items-start justify-between gap-4">
                <View className="flex-1 gap-2">
                  <Text className="text-[22px] font-extrabold text-foreground">{t('arrival.locationTitle')}</Text>
                  <Text className="text-[15px] leading-6 text-muted">{t('arrival.locationBody')}</Text>
                </View>
                {stepBadge(locationState) ? <Badge label={stepBadge(locationState)} variant={locationState === 'checked' ? 'brand' : 'muted'} /> : null}
              </View>
              <Button label={t('arrival.runCheck')} onPress={() => void runLocationCheck()} variant="secondary" />
            </Card>

            <Card className="gap-4 rounded-[32px] bg-white/72">
              <View className="flex-row items-start justify-between gap-4">
                <View className="flex-1 gap-2">
                  <Text className="text-[22px] font-extrabold text-foreground">{t('arrival.faceTitle')}</Text>
                  <Text className="text-[15px] leading-6 text-muted">{t('arrival.faceBody')}</Text>
                </View>
            ...
            <Card className="gap-3 rounded-[32px] bg-[#eef2ff]">
              <Text className="text-[18px] font-extrabold text-foreground">{t('arrival.readyTitle')}</Text>
              <Text className="text-[15px] leading-6 text-muted">{t('arrival.readyBody')}</Text>
            </Card>
            ...
            <View className="mt-auto gap-3 pb-2">
              <Button
                ...
                label={submitting ? t('arrival.processing') : t('arrival.continue')}

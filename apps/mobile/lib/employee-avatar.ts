...
function pickAvatar(pool: ImageSourcePropType[], seedParts: Array<string | null | undefined>) {
...
function inferGenderFromName(firstName: string | null | undefined, lastName: string | null | undefined) {
...
function remoteAvatarSource(value: string) {
  ...
  return { uri: trimmed } satisfies ImageSourcePropType;
}

export function resolveEmployeeAvatarSource(candidate: AvatarCandidate): ImageSourcePropType {
  ...
  return genderFallbackAvatar(candidate.gender, fallbackSeedParts, candidate.firstName, candidate.lastName);
}

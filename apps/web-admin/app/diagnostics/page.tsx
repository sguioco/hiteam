...
export default function DiagnosticsPage() {
  ...
  const [policy, setPolicy] = useState<DiagnosticsPolicy | null>(null);
  const [summary, setSummary] = useState<DiagnosticsSummaryResponse | null>(null);
  const [trends, setTrends] = useState<DiagnosticsTrendsResponse | null>(null);
  const [exportsData, setExportsData] = useState<ExportJobsResponse['items']>([]);
  const [biometricJobs, setBiometricJobs] = useState<TeamBiometricJobsResponse['items']>([]);
  const [pushDeliveries, setPushDeliveries] = useState<PushDeliveryItem[]>([]);
  ...
  const [exportMeta, setExportMeta] = useState({ total: 0, page: 1, totalPages: 1 });
  const [biometricMeta, setBiometricMeta] = useState({ total: 0, page: 1, totalPages: 1 });
  const [pushMeta, setPushMeta] = useState({ total: 0, page: 1, totalPages: 1 });

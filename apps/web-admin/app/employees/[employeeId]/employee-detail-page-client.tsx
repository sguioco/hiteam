...
export default function EmployeeCardPageClient({
  ...
  const [employee, setEmployee] = useState<EmployeeDetails | null>(initialData?.employee ?? null);
  const [history, setHistory] = useState<AttendanceHistoryResponse | null>(initialData?.history ?? null);
  const [anomalies, setAnomalies] = useState<AttendanceAnomalyResponse | null>(initialData?.anomalies ?? null);
  const [biometricHistory, setBiometricHistory] = useState<EmployeeBiometricHistoryResponse | null>(initialData?.biometricHistory ?? null);
  const [managerAccess, setManagerAccess] = useState<EmployeeManagerAccess | null>(initialData?.managerAccess ?? null);

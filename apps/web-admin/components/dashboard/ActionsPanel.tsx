  ...
  return (
    <div className="dashboard-card h-full flex flex-col overflow-y-auto scrollbar-hide">
      ...
      <div className="mb-4">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="bg-[var(--soft-danger)] text-[var(--danger)] text-[10px] font-medium px-1.5 py-0.5 rounded-full">
              {issues.length}
            </span>
          </div>
          {issuesExpanded ? (
        ...
        {issuesExpanded && (
          <div className="space-y-1.5">
            {issues.map((issue, i) => (
              <div
                key={issue.id}
                className="flex items-center justify-between p-2.5 rounded-xl bg-[var(--panel-muted)] animate-fade-in"
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-medium">{issue.employee}</p>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-[var(--soft-accent)] text-[var(--accent)] text-[10px] font-medium px-1.5 py-0.5 rounded-full">
              {documents.length}
            </span>
          </div>
          <a
        ...
        <div className="space-y-1.5 max-h-[140px] overflow-y-auto scrollbar-hide">
          {documents.map((doc, i) => (
            ...
            >
              ...
              <button className="shrink-0 rounded-md text-xs font-medium py-0.5 px-2 border border-[var(--border)] hover:bg-[var(--accent)] hover:text-white hover:border-[var(--accent)] flex items-center gap-1 transition-colors">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <Inbox className="w-4 h-4 text-[var(--warning)]" />
            <h2 className="font-semibold text-xs uppercase tracking-wider">
              Запросы в ожидании
            </h2>
            <span className="bg-[var(--soft-warning)] text-[var(--warning)] text-[10px] font-medium px-1.5 py-0.5 rounded-full">
              {requests.length}
            </span>
          </div>
          <a
        ...
        <div className="space-y-1.5">
          {requests.map((req, i) => (
            ...
            >
              ...
              <span className="text-[10px] bg-[var(--soft-warning)] text-[var(--warning)] px-2 py-0.5 rounded-full font-medium">
                Ожидание
              </span>
            </div>
          ))}

...
export const RangePresetButton = ({ value, className, children, ...props }: RangePresetButtonProps) => {
    ...
    return (
        <button
            {...props}
            className={cx(
                "cursor-pointer rounded-xl px-3 py-2 text-left text-sm font-medium text-[rgba(15,23,42,0.72)] outline-none transition duration-100 ease-linear hover:bg-[rgba(15,23,42,0.04)] hover:text-[rgba(15,23,42,0.92)] focus-visible:ring-2 focus-visible:ring-[#7c56d9]/25",
                isSelected && "bg-[rgba(124,86,217,0.08)] text-[rgba(15,23,42,0.92)]",
                className,
            )}
        >
            {children}
...
export const RangeCalendar = ({ presets, visibleDuration, showOutOfRangeDates = false, showPresetsOnDesktop = false, ...props }: RangeCalendarProps) => {
    ...
    return (
        <ContextWrapper>
            <AriaRangeCalendar
                {...props}
                className={(state) => cx("flex items-start bg-white", typeof props.className === "function" ? props.className(state) : props.className)}
                visibleDuration={{
                    months: visibleDurationMonths,
                }}
            >
                <div className="flex flex-col gap-3 bg-white px-6 py-5 md:gap-2">
                    <header className={cx("relative flex items-center", visibleDurationMonths > 1 ? "justify-start" : "justify-between")}>
                        <Button
                            ...
                            size="sm"
                            color="tertiary"
                            className="size-8 border-0 bg-transparent text-[rgba(15,23,42,0.72)] shadow-none ring-0 hover:bg-[rgba(15,23,42,0.04)] hover:text-[rgba(15,23,42,0.92)]"
                        />

                        <h2 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-sm font-semibold text-[rgba(15,23,42,0.84)]">
                        ...
                        <AriaCalendarGridHeader>
                            {(day) => (
                                <AriaCalendarHeaderCell className="border-b-4 border-transparent p-0">
                                    <div className="flex size-10 items-center justify-center text-sm font-medium text-[rgba(15,23,42,0.66)]">{day.slice(0, 2)}</div>
                ...
                {visibleDurationMonths > 1 && (
                    <div className="flex flex-col gap-3 border-l border-[rgba(15,23,42,0.08)] bg-white px-6 py-5">

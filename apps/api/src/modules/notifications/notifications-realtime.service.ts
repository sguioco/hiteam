...
import { NOTIFICATIONS_REDIS_CHANNEL, NotificationRealtimeEnvelope } from './notifications-realtime.types';
...
export class NotificationsRealtimeService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (!redisUrl) {
      this.logger.warn('REDIS_URL is not configured. Notifications realtime will use in-process delivery only.');
      return;
    }
    ...
    try {
      ...
      await this.subscriber.subscribe(NOTIFICATIONS_REDIS_CHANNEL);
      this.subscriber.on('message', (_channel: string, rawMessage: string) => {
        this.handleMessage(rawMessage);
      });

      this.redisEnabled = true;
      this.logger.log('Notifications realtime is using Redis pub/sub transport.');
    } catch (error) {
      this.logger.warn(
  ...
  async fanout(message: NotificationRealtimeEnvelope) {
    if (this.redisEnabled && this.publisher) {
      try {
        await this.publisher.publish(NOTIFICATIONS_REDIS_CHANNEL, JSON.stringify(message));
        return;
      } catch (error) {
        this.logger.warn(
  ...
  private handleMessage(rawMessage: string) {
    try {
      const message = JSON.parse(rawMessage) as NotificationRealtimeEnvelope;
      this.dispatch(message);
    } catch (error) {
      this.logger.warn(`Failed to parse notification realtime message. ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  private dispatch(message: NotificationRealtimeEnvelope) {
    if (message.type === 'notification.created') {
      this.notificationsGateway.emitNotification(message.userId, message.notification);
      this.notificationsGateway.emitUnreadCount(message.userId, message.unreadCount);
      return;
    }

...
Endpoint: GET /locations/{location_id}/clients/{client_id}/chain_attendances_statistic/{chain_id}
Version: 2.0.0
...
## Path parameters:
  ...
  - `location_id` (integer, required)
    Location ID used to authorize the request
...
## Response 200 fields (application/json):
  ...
  - `data.type` (string, required)
    Literal legacy JSON:API resource type for Chain statistics

  - `data.id` (string, required)
    Compound Booking User and Chain identifier
  ...
  - `data.attributes.company_id` (integer)

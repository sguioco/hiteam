...
Move an Appointment from this Event to another Event in the same Location.

Endpoint: PATCH /locations/{location_id}/events/{event_id}/appointments/{record_id}
Version: 2.0.0
...
## Query parameters:
  ...
  - `include` (array)
    Related resources to include in the rescheduled Appointment
...
## Request fields (application/json):
  ...
  - `reschedule_activity_id` (integer, required)
    Literal wire field for the destination Event ID

  - `with_comer` (boolean)
    Whether to retain the linked walk-in visitor

...
Update pricing, Product Items, Tags, color, comment, or capacity for an Event Appointment.

Endpoint: PUT /locations/{location_id}/events/{event_id}/appointments/{record_id}
Version: 2.0.0
...
## Path parameters:
  ...
  - `record_id` (integer, required)
    Appointment ID; record_id is the literal wire path parameter

## Query parameters:

  - `include` (array)
    Related resources to include in the updated Appointment
...
## Request fields (application/json):
  ...
  - `attendance_service_item` (object, required)

  - `attendance_service_item.cost_per_unit` (number, required)

  - `attendance_service_item.discount_percent` (number, required)

  - `attendance_service_item.manual_cost` (number, required)

  - `attendance_service_item.is_trial` (boolean)
    Whether this is a trial Service Appointment

  - `attendance_good_items` (array)
    Literal wire field for Appointment Product Items

  - `attendance_good_items.id` (integer,null, required)
    Appointment Product Item ID; null creates an item

  - `attendance_good_items.good_id` (integer, required)
    Literal wire field for Product ID

  - `attendance_good_items.unit_id` (integer, required)
    Unit ID

  - `attendance_good_items.storage_id` (integer, required)
    Inventory Location ID

  - `attendance_good_items.staff_id` (integer, required)
    Team Member ID

  - `attendance_good_items.operation_unit_type` (string, required)
    Product operation type
    Enum: "sale", "consumable"

  - `attendance_good_items.number` (string,null, required)

  - `attendance_good_items.quantity` (number, required)

  - `attendance_good_items.cost_per_unit` (number, required)

  - `attendance_good_items.manual_cost` (number, required)

  - `attendance_good_items.discount_percent` (number, required)

  - `attendance_good_items.marks` (array, required)

  - `attendance_good_items.planned_activation_date` (string,null, required)

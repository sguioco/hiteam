...
Endpoint: PATCH /company/{location_id}/booking_forms/{form_id}
Version: 1.0.0
...
## Request fields (application/json):
  ...
  - `button_animation` (boolean, required)
    Button animation enable flag

  - `button_color` (string, required)
    Button color

  - `button_position` (string, required)
    Button location on the page
    Enum: "bottom right", "bottom left", "top right", "top left"

  - `datetime_step_num` (number, required)
    Sequence number of the date and time step

  - `description` (string, required)
    appointment widget description

  - `form_position` (string, required)
  ...
  - `master_step_default` (number, required)
    team member ID

  - `master_step_hide` (boolean, required)
    team member hide flag

  - `master_step_num` (number, required)
    Sequence number of the wizard step

  - `service_step_default` (number, required)
    Service ID

  - `service_step_hide` (boolean, required)
    Service Hiding Flag

  - `service_step_num` (number, required)
    Service step sequence number

  - `show_button` (boolean, required)
    Button Visibility Step

  - `title` (string, required)
    Name of the appointment widget

  - `without_menu` (boolean, required)
    Step mode flag
